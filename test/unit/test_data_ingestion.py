...
def test_chat_ingestor_resolve_dir_uses_session_dirs(tmp_dirs: Any, stub_model_loader: Any):
    ing = ChatIngestor(
...
def test_split_chunks_respect_size_and_overlap(tmp_dirs: Any, stub_model_loader: Any):
    ing = ChatIngestor(
        temp_base="data", faiss_base="faiss_index", use_session_dirs=True
    )
...
def test_faiss_manager_add_documents_idempotent(tmp_dirs: Any, stub_model_loader: Any):
    fm = FaissManager(index_dir=pathlib.Path("faiss_index/test"))
    fm.load_or_create(
        texts=["hello", "world"], metadatas=[{"source": "a"}, {"source": "b"}]
    )
