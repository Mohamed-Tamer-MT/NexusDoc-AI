...
# ----------------------------
APP_NAME = "NexusDoc AI"
APP_TAGLINE = "Intelligent document conversation"
...
def session_status(session_id: str) -> dict[str, bool]:
    """Chat history lives in memory, so a stored session may expire after restart."""
    return {"valid": session_id in SESSIONS}
...
async def upload(
    ...
    try:
        ...
        ingestor.built_retriver(
            uploaded_files=wrapped_files, search_type="mmr", fetch_k=20, lambda_mult=0.5
        )
        ...
        return UploadResponse(
            session_id=session_id, indexed=True, message="Indexing complete with MMR"
        )
...
async def chat(req: ChatRequest) -> ChatResponse:
    ...
    try:
        ...
        rag.load_retriever_from_faiss(
            index_path=index_path, search_type="mmr", fetch_k=20, lambda_mult=0.5
        )
